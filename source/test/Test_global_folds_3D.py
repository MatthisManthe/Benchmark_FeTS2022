...
def main(args, config):
    ...
    if config["test_folds"]:
        ...
        for fold in folds:
            ...
            if config["model_file"] == "best_model.pth":
                global_model = torch.load(os.path.join(current_dir, config["model_file"]))
            else:
                global_model.load_state_dict(torch.load(os.path.join(current_dir, config["model_file"])))
            
            for name, param in global_model.named_parameters():
                print(name, param.shape)
                
            if config["multi_gpu"]:
                model_parallel = torch.nn.DataParallel(global_model)
            else:
                model_parallel = global_model        
