    ...
    if config["test_folds"]:
        ...
        for fold in folds:
            ...
            for client in samples_inst_map_train.keys():
                ...
                if config["per_institution_hp"]:
                    for idx, param in enumerate(config["param"][client]):
                        model_dir = model_dir.replace('$'+str(idx), str(param))
                        
                local_model.load_state_dict(torch.load(os.path.join(model_dir, config["model_file"].replace("#", client))))
