...
def main(args, config):
    ...
    if config["nb_gpus"] == 1:
        local_model_parallel = local_model
    else:
        local_model_parallel = torch.nn.DataParallel(local_model)
    ...
    if config["persistent_dataset"]:
        ...
        train_ds = LMDBDataset(data=samples_inst_map_train["1"], transform=train_transform, cache_dir=config["cache_dir"])
        ...
        train_ds = CacheDataset(data=samples_inst_map_train["1"], transform=train_transform)
        for i in samples_inst_map_train.keys():
    ...
    optimizer = torch.optim.SGD(local_model.parameters(), config["learning_rate"], weight_decay=config["weight_decay"])
    ...
    writer.add_graph(local_model, next(iter(train_loader))["image"].to(device))
    print(summary(local_model, next(iter(train_loader))["image"].to(device), show_input=True, show_hierarchical=True))
    ...
    for epoch in range(config["max_epochs"]):
        ...
        writer.add_scalar(f"Learning rate/Client 1/Epoch", optimizer.param_groups[0]['lr'], epoch+1)
        ...
        for (idx, batch_data) in enumerate(tqdm(train_loader)):
            ...
            optimizer.zero_grad()
            outputs = local_model_parallel(inputs)
        ...
        # lr_scheduler.step()
        epoch_loss /= len(samples_inst_map_train["1"])

        writer.add_scalar(f"Loss/train/Client 1", epoch_loss, epoch+1)
        print(f"Client 1 epoch {epoch + 1} average loss: {epoch_loss:.4f}")

        # --------------- Federated Local validation step ---------------
        if (epoch+1) % config['local_validation_interval'] == 0:
            ...
            with torch.no_grad():
                ...
                for client in samples_inst_map_train.keys():
                    ...
                    for (idx, val_data) in enumerate(tqdm(val_loader_list[client])):
                        ...
                        val_outputs = sliding_window_inference(val_inputs, config["roi_size"], batch_size, local_model_parallel, overlap=config["sliding_window_overlap"], mode="gaussian")
                    ...
                    if client == "1" and metric_d > best_mean_dice:
                        best_mean_dice = metric_d
                        best_dice_et = metric_et
                        best_dice_tc = metric_tc
                        best_dice_wt = metric_wt
                        best_epoch = epoch + 1
                        torch.save(local_model.state_dict(), os.path.join(log_dir, "best_local_model.pth"))
                        print("\nSaved new best metric local model")
                    print(
                        ...
                        f"at global epoch: {best_epoch}"
                    )
                    
                    if (epoch + 1) in config["save_epochs"]:
                        shutil.copyfile(os.path.join(log_dir, "best_local_model.pth"), os.path.join(log_dir, f"best_local_model_epoch_{epoch+1}.pth"))
                ...
                writer.add_scalar("Validation/Global Mean_dice", metric_d, epoch+1)
                writer.add_scalar("Validation/Global Tumor core dice", metric_tc, epoch+1)
                writer.add_scalar("Validation/Global Whole tumor dice", metric_wt, epoch+1)
                writer.add_scalar("Validation/Global Enhancing tumor dice", metric_et, epoch+1)
