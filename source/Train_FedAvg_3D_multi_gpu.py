...
def main(args, config):
    ...
    for comm in range(config["max_comm_rounds"]):
        ...
        for client in samples_inst_map_train.keys():
            ...
            if config["momentum_restart"]:
                lr = optimizer_list[client].param_groups[0]['lr']
                if config["optim"] == "sgdm":
                    optimizer_list[client] = torch.optim.SGD(local_models[client].parameters(), lr, momentum=config["momentum"], weight_decay=config["weight_decay"])
                elif config["optim"] == "adam":
                    optimizer_list[client] = torch.optim.Adam(local_models[client].parameters(), lr, weight_decay=config["weight_decay"])
                if config["lr_scheduler"] == "exp_decay":
                    scheduler_list[client] = torch.optim.lr_scheduler.ExponentialLR(optimizer_list[client], config["gamma"], verbose=True)
        ...
        for client in samples_inst_map_train.keys():  
            ...
            for global_param, client_param in zip(global_model.parameters(), local_models[client].parameters()):
                if config["weighted_average"]:
                    ...
                    global_param.data += client_param.data.clone() * 1.0/nb_clients
