...
def main(args, config):
    ...
    client_c = {client:[] for client in samples_inst_map_train.keys()}
    delta_c = {client:[] for client in samples_inst_map_train.keys()}
    ...
    for comm in range(config["max_comm_rounds"]):
        ...
        for client in samples_inst_map_train.keys():
            ...
            for id_group, (client_c_group, global_c_group, local_model_group, global_model_group) in enumerate(zip(client_c[client], global_c, local_models[client].parameters(), global_model.parameters())):
                delta_c[client][id_group] = -global_c_group + 1.0/num_batches/config["learning_rate"] * (global_model_group - local_model_group)
        ...
        for client in samples_inst_map_train.keys():
            for id_group, delta_control_group in enumerate(delta_c[client]):
                if config["weighted_control_variate"]:
                    global_c[id_group] += delta_control_group * len(samples_inst_map_train[client])/len(train_dict)
                else:
                    global_c[id_group] += delta_control_group / nb_clients
