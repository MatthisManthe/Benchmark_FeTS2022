...
def main(args, config):
    ...
    if config["optim"] == "sgd":
        ...
        optimizer_list = {i:torch.optim.SGD(local_models[i].parameters(), config["learning_rate"], momentum=config["momentum"], weight_decay=config["weight_decay"]) for i in samples_inst_map_train.keys()}
    ...
    # Saving local losses for PID averaging
    loss_history_dict = {client:np.ones(6, dtype=np.float32) for client in samples_inst_map_train.keys()}
    ...
    for comm in range(config["max_comm_rounds"]):
        ...
        for client in samples_inst_map_train.keys():
            ...
            for local_epoch in range(config["max_local_epochs"]):
                ...
                # Save the last average local loss for PID averaging
                if config["local_loss"] == "train" and local_epoch == config["max_local_epochs"] - 1:
                    loss_history_dict[client][:5] = loss_history_dict[client][1:].copy()
                    loss_history_dict[client][-1] = local_epoch_loss
                ...
                if config["local_loss"] == "val" or (False and (local_epoch+1) % config['local_validation_interval'] == 0):
                    ...
                    with torch.no_grad():
                        ...
                        if config["local_loss"] == "val" and local_epoch == config["max_local_epochs"] - 1:
                            loss_history_dict[client][:5] = loss_history_dict[client][1:].copy()
                            loss_history_dict[client][-1] = 1 - metric_d
                            print(loss_history_dict)
        ...
        if config["positive_difference"]:
            K = np.sum([np.max([0.,loss_history_dict[client][-2] - loss_history_dict[client][-1]]) for client in samples_inst_map_train.keys()], dtype=np.float32)
            if K == 0:
                print("\n\n\n\n\n\n\n\n\nK is zero, val stability problem ...\n\n\n\n\n\n\n")
                K = 1
        else:
            K = np.sum([loss_history_dict[client][-2] - loss_history_dict[client][-1] for client in samples_inst_map_train.keys()], dtype=np.float32)
        I = np.sum([np.sum(loss_history_dict[client]) for client in samples_inst_map_train.keys()], dtype=np.float32)
        ...
        for client in samples_inst_map_train.keys():  
            ...
            # Print weights of each client per communication round, saliency test
            if config["positive_difference"]:
                print(f"Sample size weights of client {client}: nb samples {len(samples_inst_map_train[client])}, final weight {len(samples_inst_map_train[client])/len(train_dict)}, \n\
                      Loss weight: cost difference {np.max([np.float32(0.),loss_history_dict[client][-2] - loss_history_dict[client][-1]])}, final weight {np.max([np.float32(0.),loss_history_dict[client][-2] - loss_history_dict[client][-1]]) / K} \n\
                      Cumulative loss weight: cum cost {np.sum(loss_history_dict[client])}, final weight {(np.sum(loss_history_dict[client])) / I}")
            else:
                print(f"Sample size weights of client {client}: nb samples {len(samples_inst_map_train[client])}, final weight {len(samples_inst_map_train[client])/len(train_dict)}, \n\
                      Loss weight: cost difference {loss_history_dict[client][-2] - loss_history_dict[client][-1]}, final weight {(loss_history_dict[client][-2] - loss_history_dict[client][-1]) / K} \n\
                      Cumulative loss weight: cum cost {np.sum(loss_history_dict[client])}, final weight {(np.sum(loss_history_dict[client])) / I}")
            ...
            for global_param, client_param in zip(global_model.parameters(), local_models[client].parameters()):
                if config["positive_difference"]:
                    global_param.data += client_param.data.clone() * \
                        (config["alpha_agg"] * len(samples_inst_map_train[client])/len(train_dict) + \
                        config["beta_agg"] * np.max([np.float32(0.),loss_history_dict[client][-2] - loss_history_dict[client][-1]]) / K + \
                        config["gamma_agg"] * (np.sum(loss_history_dict[client])) / I)
                else:
                    global_param.data += client_param.data.clone() * \
                        (config["alpha_agg"] * len(samples_inst_map_train[client])/len(train_dict) + \
                        config["beta_agg"] * (loss_history_dict[client][-2] - loss_history_dict[client][-1]) / K + \
                        config["gamma_agg"] * (np.sum(loss_history_dict[client])) / I)
