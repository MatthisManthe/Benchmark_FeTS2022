...
def main(args, config):
    ...
    # Initialize the global and local models as the best obtained federated model.
    global_model.load_state_dict(torch.load(os.path.join(config["model_dir"], config["model_file"])))
        ...
        for local_epoch in range(config["max_local_epochs"]):
            ...
            for (idx, batch_data) in enumerate(tqdm(train_loader_list[client])):
                ...
                for local_param, global_param in zip(local_models[client].parameters(), global_model.parameters()):
                    local_param.grad.data += config["lambda"] * (local_param.data - global_param.data)
