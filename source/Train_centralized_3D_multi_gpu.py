...
from monai.data import DataLoader, decollate_batch, CacheDataset, Dataset, PersistentDataset, LMDBDataset
...
def main(args, config):
    ...
    if config["pretrained_model"]:
        model.load_state_dict(torch.load(config["pretrained_model_file"]))
    ...
    for epoch in range(config["max_epochs"]):
        ...
        for (idx, batch_data) in enumerate(tqdm(train_loader)):
            ...
            iteration += 1
            writer.add_scalar("Learning rate/Iteration", optimizer.param_groups[0]['lr'], iteration)
        ...
        writer.add_scalar("One_epoch_train_time", t_end-t_start, epoch+1)
        ...
        if (epoch) % config["validation_interval"] == 0:
            ...
            with torch.no_grad():
                ...
                for (idx, val_data) in enumerate(tqdm(val_loader)):
                    ...
                    val_outputs = sliding_window_inference(val_inputs, config["roi_size"], config["nb_gpus"]*config["batch_size_per_gpu"], model_parallel, overlap=config["sliding_window_overlap"], mode="gaussian")
        ...
        torch.cuda.synchronize()
        t_end = time.time()
        writer.add_scalar("One_validation_time", t_end-t_start, epoch+1)
